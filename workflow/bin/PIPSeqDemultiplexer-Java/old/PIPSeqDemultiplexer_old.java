...
public class PIPSeqDemultiplexer {
	...
	public static void main(String[] args) {
		...
		if (args.length != 3) {
			System.out.println("Usage: FastqFilter <barcodesToKeep> <sampleName> <dir_to_fastqs>");
			return;
		}
		...
		Map<String, List<Path>> pairedFiles = Files.list(directory)
			.filter(file -> Files.isRegularFile(file) && (file.toString().endsWith("_R1.fastq") || file.toString().endsWith("_R2.fastq")))
			.collect(Collectors.groupingBy(file -> file.getFileName().toString().replaceAll("_R[12]\\.fastq$", "")));
		...
		for (Map.Entry<String, List<Path>> entry : pairedFiles.entrySet()) {
			...
			if (pair.size() == 2) {
				...
				String fastqR1 = r1.toAbsolutePath().toString();
				String fastqR2 = r2.toAbsolutePath().toString();
				System.out.println(fastqR1);
				System.out.println(fastqR2);
				demultiplexPips(r1, r2, barToKeep, sampleName, baseName);
