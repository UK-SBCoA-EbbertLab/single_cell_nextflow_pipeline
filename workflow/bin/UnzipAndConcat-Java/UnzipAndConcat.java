...
public class UnzipAndConcat {
	public static void main(String[] args) {
		...
		sampleToFolderMap
				.forEach((folder, sampleId) -> System.out.println("SampleID: " + sampleId + "   folder: " + folder));
	...
	private static void extractGzFiles(String directory) throws IOException {
		Files.walk(Paths.get(directory)).filter(Files::isRegularFile)
				.filter(path -> path.toString().endsWith(".fastq.gz")).filter(path -> {
					try {
	...
	private static void concatenateFastqFiles(String directory, String sampleId) throws IOException {
		...
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(outputFile, true))) {
			Files.walk(Paths.get(directory)).filter(Files::isRegularFile)
					.filter(path -> path.toString().endsWith(".fastq")).forEach(path -> {
